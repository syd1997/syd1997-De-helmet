...
def nms_method(boxes, scores, labels, method=3, iou_thr=0.5, sigma=0.5, thresh=0.001, weights=None):
    ...
    for i in range(len(boxes)):
        if len(boxes[i]) != len(scores[i]) or len(boxes[i]) != len(labels[i]):
            print('Check length of boxes and scores and labels: {} {} {} at position: {}. Boxes are skipped!'.format(
...
def nms(boxes, scores, labels, iou_thr=0.5, thresh=0.001, weights=None):
    """
    ...
    :return:
    """
    return nms_method(boxes, scores, labels, method=3, iou_thr=iou_thr, thresh=thresh, weights=weights)
